...
import { validateDbInstance, validateSchema, ensureMethodNameIsNotReserved, validateCustomMethods } from '../utils/mongomod.js';
...
describe('utils/connection.ts', () => {
    describe('validateOptions', () => {
        ...
        it('should throw on null options', () => {
            expect(() => validateOptions(null)).toThrow(MmValidationError);
        });
        it('should throw on undefined options', () => {
            expect(() => validateOptions(undefined)).toThrow(MmValidationError);
        });
    ...
    describe('validateConnectCallback', () => {
        ...
        it('should throw if callback is not a function or undefined', () => {
            expect(() => validateConnectCallback('not a function')).toThrow(MmValidationError);
            expect(() => validateConnectCallback(123)).toThrow(MmValidationError);
            expect(() => validateConnectCallback({})).toThrow(MmValidationError);
            expect(() => validateConnectCallback(null)).toThrow(MmValidationError);
        });
        it('should throw with proper error code for invalid callback', () => {
            try {
                validateConnectCallback('invalid');
            }
            catch (err) {
                expect(err).toBeInstanceOf(MmValidationError);
...
describe('utils/controller.ts', () => {
    ...
    describe('validateControllerCollection', () => {
        ...
        it('should throw if collection is null', () => {
            expect(() => validateControllerCollection(mockDb, null)).toThrow(MmControllerError);
        });
        it('should throw if collection is undefined', () => {
            expect(() => validateControllerCollection(mockDb, undefined)).toThrow(MmControllerError);
        });
        it('should throw if collection is not a string', () => {
            expect(() => validateControllerCollection(mockDb, 123)).toThrow(MmControllerError);
        ...
        it('should throw with null dbName when db has no dbName', () => {
            const dbWithoutName = { dbName: null };
            try {
...
describe('utils/mongomod.ts', () => {
    ...
    describe('validateCustomMethods', () => {
        it('should return true for null customs', () => {
            expect(validateCustomMethods(null)).toBe(true);
        });
        ...
        it('should throw if customs is not an object', () => {
            expect(() => validateCustomMethods('string')).toThrow(MmValidationError);
            expect(() => validateCustomMethods(123)).toThrow(MmValidationError);
        });
        it('should throw if customs is an array', () => {
            expect(() => validateCustomMethods(['method'])).toThrow(MmValidationError);
        });
