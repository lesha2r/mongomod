...
describe('mongomod.createModel validation', () => {
    it('throws when db is not a MongoConnection instance', () => {
        expect(() => mongomod.createModel({ db: {}, collection: 'users' })).toThrow();
    });
    it('throws when schema is not a MongoSchema instance', () => {
        expect(() => mongomod.createModel({ db, collection: 'users', schema: {} })).toThrow();
    });
    it('throws when customs is not an object', () => {
        expect(() => mongomod.createModel({ db, collection: 'users', customs: 'bad' })).toThrow();
    });
